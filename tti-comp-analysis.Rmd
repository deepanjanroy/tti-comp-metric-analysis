...
df <- read_csv("out/mobile-10k.csv") %>%
  ...
  mutate(tbt = `SumOfQueuingTimeGT50_p1.0-FCP-Interactive`,
         TBT = `SumOfQueuingTimeGT50_p1.0-FCP-Interactive`)  # Just don't want to break things right now.
```
...
Understanding the TTI distribution. First, some summary stats:
```{r}
df %>% 
  summarize(mean(TTI), quantile(TTI, 0.1), quantile(TTI, 0.5), quantile(TTI, 0.9), quantile(TTI, 0.99))
```
...
```{r tti}
p <- df %>% 
  ggplot(aes(x = `TTI`, y = tbt, text = page_name)) +
  labs(x = 'TTI (ms)', y = 'Sum of Queueing Time > 50ms between FCP and TTI') +
  geom_point()
ggplotly(p)
```
...
p <- df %>%
  ...
  mutate(tti_bucket = cut(TTI,
                          ...
                          labels=c(paste0(seq(0, 130, 10),
                                          "-",
                                          seq(10, 140, 10),
                                          "s")))) %>%
  ggplot(aes(x = tti_bucket,
             y = TBT + 1)) +
  geom_violin() +
  scale_y_log10() + 
...
df %>%
  ...
  mutate(tti_bucket = cut(TTI,
                          ...
                          labels=c(paste0(seq(0, 130, 10),
                                          ...
                                          "s")))) %>%
  group_by(tti_bucket) %>%
  summarize(count = n(), TBT10thPct = quantile(TBT, .1), TBT90thPct = quantile(TBT, .9)) %>%
  select(tti_bucket, count, TBT10thPct, TBT90thPct)
```
